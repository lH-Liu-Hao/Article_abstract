...
from Utility.Baidu_Message.Baidu_article.gne.utils import iter_node
from Utility.Baidu_Message.Baidu_article.gne.defaults import USELESS_TAG
...
class ContentExtractor:
    ...
    def extract(self, selector, content_rule, html):
        if content_rule:
            body = selector.xpath(content_rule)
            if body:
                body = body[0]
            else:
                doc = etree.HTML(html)
                body = doc.xpath(content_rule)
                if body:
                    body = body[0]
                else:
                    return ''
